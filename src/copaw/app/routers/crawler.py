...
async def _proxy(
    ...
    *,
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "crawler_error", "detail": str(e)},
            status_code=502,
        )
...
async def trigger_search(request: Request):
    ...
    return await _proxy("POST", "/api/scraping/search", json_body=body)
...
async def trigger_detail(request: Request):
    ...
    body = await request.json()
    return await _proxy("POST", "/api/scraping/detail", json_body=body)
...
async def trigger_batch_detail(request: Request):
    ...
    body = await request.json()
    return await _proxy("POST", "/api/scraping/batch-detail", json_body=body)


@router.delete("/jobs/{job_id}")
async def cancel_job(job_id: str):
    """Cancel a running/pending job."""
    return await _proxy("DELETE", f"/api/scraping/jobs/{job_id}")
...
async def mark_read(notification_id: str):
    ...
    return await _proxy("PATCH", f"/api/notifications/{notification_id}/read")
...
async def delete_notification(notification_id: str):
    ...
    return await _proxy(
        "DELETE", f"/api/notifications/{notification_id}"
    )
...
async def generate_listings(request: Request):
    ...
    if not asins:
        return JSONResponse(
            content={"error": "asins_required"}, status_code=400,
        )
    ...
    for asin in asins:
        try:
            # Fetch product from crawler API
            async with httpx.AsyncClient(timeout=_TIMEOUT, trust_env=False) as client:
                resp = await client.get(
                    _crawler_url(f"/api/products/{asin}")
                )
            if resp.status_code != 200:
                skipped.append({"asin": asin, "reason": "not_found"})
                continue
            ...
            listing = Listing(
                asin=asin,
                title=title,
                bullet_points=product.get("about_this_item") or [],
                ...
                marketplace="",
                status="draft",
                source_url=product.get("product_url") or "",
            )
            created = await repo.create_listing(listing)
            generated.append(created.model_dump(mode="json"))
            ...
            skipped.append({"asin": asin, "reason": str(e)})
